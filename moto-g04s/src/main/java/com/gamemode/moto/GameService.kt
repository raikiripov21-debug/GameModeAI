 ...
 * AdaptiveEngine integrado con parametros independientes (Unisoc T606).
 */
class GameService : Service() {
    ...
    override fun onCreate() {
        ...
        wakeLock = (getSystemService(Context.POWER_SERVICE) as PowerManager)
            .newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, "GameModeAI_Moto::WakeLock")
        wakeLock?.acquire(20 * 60 * 1000L)
        AdaptiveEngine.restoreState(this)
    }

    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        startForeground(NOTIF_ID, buildNotif("Iniciando Moto G04s…", "Motor Moto arrancando"))
    ...
    private fun createChannel() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            val ch = NotificationChannel(CHANNEL_ID, "Game Mode AI Moto", NotificationManager.IMPORTANCE_MIN)
